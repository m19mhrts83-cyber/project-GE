...
**単独の地場店** → 従来どおり送ってよい。  
**同一系列・同一グループの複数店舗** → **同じ問合せ経路に二重送信しない**（心証対策）。

### 系列とみなす目安（いずれかで該当）

1. 社名に **同一ブランド・同一本社名**（例: 大京穴吹、センチュリー21、ピタットハウス、東急リバブル、住友不動産販売 等）
2. **`contact_url` が同一**（本部共通フォーム・同一パス。店舗違いでも同じ送信先）
3. **`url` / `contact_url` の親ドメインが同じ** かつ 問合せが **本部一括**（店舗ページから同じ `/inquiry/` 等へ誘導）
4. JSON の `group_key` / `chain_id` が同じ（Jarvis が付与した場合）

**単独店の目安**: 社名に系列ブランドが無い独立仲介、オーナー個人系、`group_key` が unique / 空。

### 送信前の確認（各社・必須）

1. 当日 JSON の `vendors` 内で、**既に本セッション送信済み**の系列と重複していないか
2. JSON に `contacted_vendors` / `group_contacted` があれば、**過去に個人送信済み（contacted/replied）** の系列と重複していないか
3. **`contact_url` を開き**、店舗別フォームか本部共通フォームかを1行判定
4. **重複疑い** → **送信せず** スキップ報告（下記）。**単独と判断** → 送信可。**不明** → 松野に1行確認（「〇〇系列と△△店は同一経路と見ます。スキップでよいですか？」）

### 重複時の動作（送信禁止）

- フォーム入力・送信 **しない**
- 報告例:
  - `chubu-005 スキップ: 系列重複（大京穴吹）。chubu-001 と同一 contact_url。心証のため1経路のみ`
  - Mac 記録: `--mark chubu-005 --status skip --note "系列重複: 大京穴吹 chubu-001済・同一問合せ経路"`
- 当日の Phase 上限は **送信成功件数** で数える。スキップ分は **別店・単独店で埋めない**（リスト順の次候補は Jarvis の次回 `--next` 任せ。勝手にリスト外を追加しない）

### 例外（松野が明示したときのみ）

- 「系列でもこの2店は別経路なので送って」
- 「本部と店舗の両方に送る」（明示指示がある場合のみ）
...
0. **系列チェック**（上記）。NG ならスキップ報告して次へ
1. `contact_url` があれば優先。なければ `url` から問合せフォームを探す
2. 下記 **approved A'-v2 標準文面** を使用（list_region でエリア差替）
3. フォーム各欄に入力（名前・フリガナ・メール・電話・本文）。**電話・住所は JSON の `form_contact` があればそれを使う**。無ければ松野に1行確認（jarvis_private を Bot が直接読むことはない）
...
6. 本日分終了後、サマリー（成功/失敗/スキップ理由）＋ **Phase 昇格提案1行**
