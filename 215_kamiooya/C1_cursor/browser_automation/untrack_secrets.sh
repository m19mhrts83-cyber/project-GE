#!/bin/bash
# .env と config_*.yaml を Git の追跡から外す（ファイルは削除しない）。
# リポジトリルート（215_神・大家さん倶楽部）で実行してください。
# 例: cd "/path/to/215_神・大家さん倶楽部" && ./C1_cursor/browser_automation/untrack_secrets.sh

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"
if git rm --cached C1_cursor/browser_automation/.env C1_cursor/browser_automation/config_nichinoken.yaml C1_cursor/browser_automation/config_tokairokin.yaml 2>/dev/null; then
