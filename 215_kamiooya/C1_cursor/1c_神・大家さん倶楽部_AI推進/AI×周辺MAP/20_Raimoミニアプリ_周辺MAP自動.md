...
- **生成API**: Pagesは静的UIのみ。フル自動はローカル `scripts/shuhen_auto_server.py`、または `?api=https://…` 付き
...
| 役割 | 置き場 |
|---|---|
| UI（HTML／JS） | Pages（上記 URL） |
| 入口 | Raimo マイミニアプリ（番号ピン・MyPrompt と同列） |
| 生成オーケストレーション | ローカルサーバ or 別APIホスト |
