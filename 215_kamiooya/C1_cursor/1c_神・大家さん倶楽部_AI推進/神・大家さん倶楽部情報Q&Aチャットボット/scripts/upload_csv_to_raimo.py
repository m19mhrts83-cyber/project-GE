...
# -*- coding: utf-8 -*-
"""
Raimo アプリに管理者ログインし、CSV取込を自動実行する。

必須環境変数（RAIMO_* が正。LIMO_* は後方互換）:
  RAIMO_APP_URL
  1段目（ライモBiz等のポータル）:
    RAIMO_PORTAL_EMAIL / RAIMO_PORTAL_PASSWORD（推奨）
    または RAIMO_ADMIN_EMAIL / RAIMO_ADMIN_PASSWORD（PORTAL 未設定時に1段目へ使用）
  2段目（ミニアプリのログイン）:
    RAIMO_APP_EMAIL / RAIMO_APP_PASSWORD（ポータル通過後にアプリのログイン画面が出る場合に必須）

任意環境変数:
  RAIMO_HEADLESS=1/0 (既定: 1)
  RAIMO_UPLOAD_TIMEOUT_SEC (既定: 1800)
  RAIMO_SLOW_MO_MS (既定: 0)
  RAIMO_LOGIN_WAIT_MS (ログイン画面の要素待ち ms、既定: 120000)
...
使い方:
  python3 upload_csv_to_raimo.py --csv /path/to/delta.csv
"""
...
def _is_page_crash_error(exc: BaseException) -> bool:
    """Chromium の Page crashed / Target crashed / Target closed 系（CI の /dev/shm 不足で起きやすい）。"""
    msg = str(exc).lower()
    needles = (
        "page crashed",
        "target crashed",  # Page.screenshot: Target crashed
        "target closed",
...
def _safe_screenshot(page, dest: Path, *, timeout_ms: int = 15000) -> bool:
    """
    ベストエフォートのスクリーンショット。例外は出さない。

    週次の差分CSVは結果ページが巨大になりやすく、full_page=True だと
    GitHub Actions 上で `Page.screenshot: Target crashed` になり、
    取込成功後にジョブ全体が失敗していた。
    """
    if page is None or dest is None:
        return False
    dest = Path(dest)
    try:
        dest.parent.mkdir(parents=True, exist_ok=True)
        page.screenshot(path=str(dest), full_page=False, timeout=timeout_ms)
        print(f"スクリーンショット保存: {dest}", flush=True)
        return True
    except Exception as e:
        print(
            f"警告: スクリーンショット失敗（取込成否には影響しません）: {e}",
            flush=True,
        )
        try:
            html_path = dest.with_suffix(".html")
            html = page.content()
            html_path.write_text(html[:200_000], encoding="utf-8", errors="replace")
            print(f"HTMLダンプ保存: {html_path}", flush=True)
        except Exception:
            pass
        return False
...
def _chromium_launch_args() -> list[str]:
    """
    GitHub Actions 等の共有ランナーでは /dev/shm が小さく、
    Chromium が CSV 取込中に Page crashed しやすい。
    """
    args = [
    ...
    extra = (os.environ.get("RAIMO_CHROMIUM_ARGS") or "").strip()
    if extra:
...
# Raimo 正本 / LIMO_* は後方互換エイリアス（GitHub Secrets・既存 .env 用）
_RAIMO_ENV_ALIASES = (
    ...
    ("RAIMO_HEADLESS", "LIMO_HEADLESS"),
    ("RAIMO_UPLOAD_TIMEOUT_SEC", "LIMO_UPLOAD_TIMEOUT_SEC"),
    ("RAIMO_SLOW_MO_MS", "LIMO_SLOW_MO_MS"),
    ("RAIMO_LOGIN_WAIT_MS", "LIMO_LOGIN_WAIT_MS"),
    ("RAIMO_POST_LOGIN_WAIT_MS", "LIMO_POST_LOGIN_WAIT_MS"),
    ("RAIMO_TRY_COMMUNITY_REFRESH", "LIMO_TRY_COMMUNITY_REFRESH"),
    ("RAIMO_COMMUNITY_REFRESH_TIMEOUT_SEC", "LIMO_COMMUNITY_REFRESH_TIMEOUT_SEC"),
    ("RAIMO_NOTIFY_MACOS", "LIMO_NOTIFY_MACOS"),
    ("RAIMO_USER_AGENT", "LIMO_USER_AGENT"),
    ("RAIMO_FAIL_OPEN", "LIMO_FAIL_OPEN"),
)


def apply_raimo_env_aliases() -> None:
    """RAIMO_* が空なら LIMO_* をコピーする（逆はしない）。"""
    for raimo_key, limo_key in _RAIMO_ENV_ALIASES:
        if not (os.environ.get(raimo_key) or "").strip():
            legacy = (os.environ.get(limo_key) or "").strip()
            if legacy:
                os.environ[raimo_key] = legacy
...
def maybe_macos_notify_done(exit_code: int, detail: str) -> None:
    ...
    Linux 等では何もしない。RAIMO_NOTIFY_MACOS=0 で無効。
    """
    if sys.platform != "darwin":
        return
    if not get_env_bool("RAIMO_NOTIFY_MACOS", True):
        return
    d = (detail or "").strip()
    if len(d) > 280:
        d = d[:277] + "..."
    ...
    if exit_code == 0:
        title = "Raimo CSV取込 完了"
    elif exit_code == 1:
        title = "Raimo CSV取込 完了（NGあり）"
    else:
        title = "Raimo CSV取込 失敗"
...
def _fill_raimo_login_on_frame(fr, email: str, password: str, timeout_ms: int) -> None:
...
def _fill_raimo_login(page, email: str, password: str, timeout_ms: int) -> None:
    ...
    while time.time() < deadline:
        for fr in _all_frames(page):
            if not _login_email_field_visible_in_frame(fr):
                continue
            try:
                _fill_raimo_login_on_frame(fr, email, password, timeout_ms)
                return
            except Exception as e:
                last_err = e
...
def try_resolve_portal_credentials() -> tuple[str, str] | None:
    ...
    pe = os.environ.get("RAIMO_PORTAL_EMAIL", "").strip()
    pp = os.environ.get("RAIMO_PORTAL_PASSWORD", "").strip()
    if pe or pp:
        if not pe or not pp:
            raise RuntimeError(
                "RAIMO_PORTAL_EMAIL と RAIMO_PORTAL_PASSWORD は、どちらも設定するか、"
                "どちらも空にしてください（片方だけは不可）。"
            )
        return pe, pp
    ae = os.environ.get("RAIMO_ADMIN_EMAIL", "").strip()
    ap = os.environ.get("RAIMO_ADMIN_PASSWORD", "").strip()
    if ae or ap:
        if not ae or not ap:
            raise RuntimeError(
                "RAIMO_ADMIN_EMAIL と RAIMO_ADMIN_PASSWORD は、どちらも設定するか、"
                "どちらも空にしてください（片方だけは不可）。"
            )
        return ae, ap
    return None
...
def resolve_portal_credentials() -> tuple[str, str]:
    ...
    if creds is None:
        raise RuntimeError(
            "ポータル認証が未設定です。"
            "RAIMO_PORTAL_* / RAIMO_ADMIN_* を設定するか、"
            "RAIMO_APP_EMAIL / RAIMO_APP_PASSWORD で公開URLに直接ログインしてください。"
        )
    return creds


def authenticate_raimo_page(
    page,
    *,
    app_email: str,
    app_password: str,
    ...
    初期表示がミニアプリのログイン画面のときは RAIMO_APP_* を優先する。
    """
    ...
    if app_email and app_password:
        ...
        _fill_raimo_login(page, app_email, app_password, login_wait_ms)
    ...
    if portal_creds is None:
        raise RuntimeError(
            "ログインが必要ですが、RAIMO_APP_EMAIL / RAIMO_APP_PASSWORD が未設定です。"
            "公開URL用のアカウントを scripts/.env に設定してください。"
        )

    print("1段目（ポータル）ログインを実行します", flush=True)
    portal_email, portal_password = portal_creds
    _fill_raimo_login(page, portal_email, portal_password, login_wait_ms)
    ...
    if not app_email or not app_password:
        raise RuntimeError(
            ...
            "RAIMO_APP_EMAIL / RAIMO_APP_PASSWORD が未設定です。"
        )
...
def wait_import_finished(ctx, timeout_sec: int) -> str:
    ...
    while time.time() < deadline:
        try:
            ...
            if _is_page_crash_error(e):
                raise PwError(f"CSV取込待ち中にページがクラッシュしました: {e}") from e
            raise
        # page.wait_for_timeout は CDP 経由のためクラッシュ時に分かりにくい。
        # 待ち自体は time.sleep で行い、次回ループで DOM を再読取する。
        time.sleep(2)
...
def try_community_info_refresh(page, timeout_sec: int) -> tuple[bool, str]:
    """
    アプリ本体 HTML には無いが、Raimo 管理シェル等に「コミュニティ情報の最新化」ボタンがある場合にクリックする。
    見つからなければ (False, 理由)。
    """
    if not get_env_bool("RAIMO_TRY_COMMUNITY_REFRESH", True):
        return False, "RAIMO_TRY_COMMUNITY_REFRESH=0 のためスキップ"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Raimo管理画面でCSV取込を自動実行")
    ap.add_argument("--csv", required=True, help="アップロードするCSVパス")
    ap.add_argument(
        "--timeout-sec",
        type=int,
        default=int(os.environ.get("RAIMO_UPLOAD_TIMEOUT_SEC", "1800")),
    ...
    app_url = get_required_env("RAIMO_APP_URL")
    portal_creds = try_resolve_portal_credentials()
    app_email = os.environ.get("RAIMO_APP_EMAIL", "").strip()
    app_password = os.environ.get("RAIMO_APP_PASSWORD", "").strip()
    if not ((app_email and app_password) or portal_creds):
        raise RuntimeError(
            "Raimo 認証情報が不足しています。"
            "公開URL運用: RAIMO_APP_EMAIL / RAIMO_APP_PASSWORD。"
            "従来の2段: RAIMO_PORTAL_*（+ 必要なら RAIMO_APP_*）。"
        )
    headless = get_env_bool("RAIMO_HEADLESS", True)
    slow_mo = int(os.environ.get("RAIMO_SLOW_MO_MS", "0"))
    ...
    with sync_playwright() as p:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                # 取込本体は完了。以降はベストエフォート
                # （巨大な importResult ページでの screenshot crash を失敗にしない）
                try:
                    refresh_timeout = int(
                    ...
                    print(f"コミュニティ最新化: 例外のためスキップ ({e})", flush=True)
