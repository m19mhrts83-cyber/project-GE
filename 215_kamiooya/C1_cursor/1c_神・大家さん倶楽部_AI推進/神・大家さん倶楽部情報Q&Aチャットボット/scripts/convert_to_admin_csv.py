...
出力ヘッダ:
  コメントID,投稿日時,投稿者名,投稿者メール,コメント内容,親コメントID,IP アドレス,ユーザーエージェント,ソース
  （ソース: app.js の CSV 取込で source_type に対応。WeStudy 取り込み時は神大家コミュニティと誤判定されないよう既定で WeStudy）
"""
...
def row_to_admin(row: dict) -> dict | None:
    ...
    src = get_cell(row, "ソース", "source_type", "sourceType", "データソース")
    if not src:
        src = "WeStudy"
...
def choose_better_admin_row(old_row: dict, new_row: dict) -> dict:
    """同一コメントIDが複数ある場合は、日時・本文情報がより豊富な行を優先する。"""
    if not old_row:
        return new_row
    old_has_date = bool((old_row.get("投稿日時") or "").strip())
    new_has_date = bool((new_row.get("投稿日時") or "").strip())
    if old_has_date != new_has_date:
        return new_row if new_has_date else old_row

    old_body_len = len((old_row.get("コメント内容") or "").strip())
    new_body_len = len((new_row.get("コメント内容") or "").strip())
    if old_body_len != new_body_len:
        return new_row if new_body_len > old_body_len else old_row

    old_author = len((old_row.get("投稿者名") or "").strip())
    new_author = len((new_row.get("投稿者名") or "").strip())
    if old_author != new_author:
        return new_row if new_author > old_author else old_row

    # 同等なら後勝ち（再スクレイプの最新状態を優先）
    return new_row
            ...
            with open(fp, "r", encoding="utf-8-sig", newline="") as f:
                ...
                for row in reader:
                    ...
                    if not admin:
                        continue
                    cid = admin["コメントID"]
                    by_id[cid] = choose_better_admin_row(by_id.get(cid), admin)
