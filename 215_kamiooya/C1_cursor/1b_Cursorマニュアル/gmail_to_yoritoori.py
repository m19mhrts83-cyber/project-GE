...
※ 開封済み（既読）メールも「漏れ確認」で拾われる。先に開封してから実行しても問題ない。
...
def build_service_for_token(token_path_for_account: Path):
    ...
    if not creds or not creds.valid:
        ...
        print(f"token を保存しました: {token_path_for_account.name}")
        sys.stdout.flush()
...
def parse_yoritoori_existing():
    """各やり取り.md から既存エントリの (partner_name, subject, date_yyyymmdd) を抽出。"""
    existing = set()
    for folder in base_path.iterdir():
        ...
        for block in blocks:
            m = re.search(
                r"### (\d{4}/\d{2}/\d{2})(?:\s+(\d{1,2}:\d{2}))?｜([^｜]+)｜相手から返信",
                block,
            )
            if not m:
                continue
            date_part = m.group(1)
            partner_name = (m.group(3) or "").strip()
            subject = ""
            sm = re.search(r"\*\*件名\*\*:\s*(.+)", block)
            if sm:
                subject = sm.group(1).strip()
            key = (partner_name, subject, date_part.replace("/", "-"))
            existing.add(key)
    return existing


def parse_yoritoori_existing_sent():
    """各やり取り.md から既存の「自分から送信」エントリの (partner_name, subject, date_yyyymmdd) を抽出。重複判定用。"""
    existing = set()
    for folder in base_path.iterdir():
        if not folder.is_dir():
            continue
        ...
        for block in blocks:
            m = re.search(
                r"### (\d{4}/\d{2}/\d{2})(?:\s+(\d{1,2}:\d{2}))?｜([^｜]+)｜自分から送信",
                block,
            )
            if not m:
                continue
            ...
            if sm:
                subject = sm.group(1).strip()
            existing.add((partner_name, subject, date_part))
    return existing
...
def process_message(service, msg, email_to_partner, mark_read=True):
    ...
    attachment_names = []
    try:
        attachment_names = save_attachments(service, msg["id"], payload, partner["folder"], date_str)
...
def main():
    ...
    parser.add_argument(
        "--import-from",
        metavar="EMAIL",
        help="指定差出人（from）の直近メールを一括取り込み（初回用途。既読/未読問わず）",
    )
    parser.add_argument("--days", type=int, default=30, help="--import-from の対象期間（日）。デフォルト30日")
    parser.add_argument(
        "--import-from-to-partner",
        metavar="PARTNER",
...
def _run_include_read(service, email_to_partner, existing=None):
    ...
    for msg in messages:
        ...
        key = (partner["name"], subject.strip(), date_key)
        if key in existing:
            continue
...
def _run_include_sent(service, email_to_partner, existing_sent=None):
    ...
    for msg in messages:
        ...
        if append_sent_to_yoritoori(partner["folder"], partner["name"], date_str, subject.strip(), body, attachment_names):
            appended += 1
...
def _run_add_at(service, email_to_partner, add_at_str):
    ...
    try:
        ...
        try:
            ...
            print(
                f"エラー: 時刻形式が不正です。YYYY-MM-DD HH:MM または HH:MM を指定してください: {add_at_str}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _run_import_from(service, email_to_partner, from_email, days):
    ...
    query = f"from:{from_email} newer_than:{int(days)}d"
    result = service.users().messages().list(userId="me", q=query, maxResults=500).execute()
    messages = result.get("messages", [])
    if not messages:
        print(f"直近{days}日に {from_email} からのメールはありませんでした。")
        sys.stdout.flush()
        return 0
    ...
    if appended > 0:
        print(f"{partner['name']}：{appended} 件のメールをやり取りに追記しました。（直近{days}日／from:{from_email}）")
    else:
        print(f"{partner['name']}：追記対象はありませんでした。（直近{days}日／from:{from_email}）")
    sys.stdout.flush()
    return appended


def _run_import_from_to_partner(service, partners, partner_name_or_folder, from_email, days):
    """
    指定 from の直近days日分を、指定パートナーのフォルダへ強制取り込みする。
    - 例: 自分のGmailから「転送/共有」されたパートナー関連メールを、初回だけまとめて取り込む用途
    """
    from_email = (from_email or "").strip().lower()
    if not from_email:
        print("エラー: --import-from-to-partner 使用時は --import-from EMAIL も指定してください。", file=sys.stderr)
        sys.exit(1)

    partner_name_or_folder = (partner_name_or_folder or "").strip()
    partner = None
    for p in partners:
        if p.get("name") == partner_name_or_folder or p.get("folder") == partner_name_or_folder:
            partner = p
            break
    if not partner:
        print(f"エラー: パートナー '{partner_name_or_folder}' が見つかりません。", file=sys.stderr)
        sys.exit(1)
    ...
    # 指定パートナーのフォルダだけを対象に既存判定（他パートナーと混ざらないように）
    ...
    blocks = re.split(r"\n---\s*\n", content)
    for block in blocks:
        m = re.search(r"### (\d{4}/\d{2}/\d{2}).*｜[^｜]+｜相手から返信", block)
        if not m:
            continue
        date_part = m.group(1).replace("/", "-")
        subject = ""
    ...
    for msg in messages:
        ...
        if append_to_yoritoori(partner["folder"], partner["name"], date_str, body, attachment_names, subject.strip()):
            appended += 1
            existing.add(key)
            log_msg = f"追記: {partner['name']} (from:{from_email}) - {subject[:40]}..."
            if attachment_names:
                log_msg += f" [添付{len(attachment_names)}件]"
            print(log_msg)

    if appended > 0:
        print(f"{partner['name']}：{appended} 件をやり取りに追記しました。（直近{days}日／from:{from_email}）")
    else:
        print(f"{partner['name']}：追記対象はありませんでした。（直近{days}日／from:{from_email}）")
    sys.stdout.flush()
    return appended
...
def main():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                # refresh token が失効している場合はブラウザで再認証
                creds = None
        if not creds or not creds.valid:
            flow = InstalledAppFlow.from_client_secrets_file(str(credentials_path), SCOPES)
            creds = flow.run_local_server(port=0)
    ...
    if args.add_at:
        ...
        # 通常: 漏れ確認（既読含む）→ 未読処理 → 送信トレイの漏れ確認
        # 漏れ確認を先に実行することで、先に開封したメールも確実に拾う
        print("漏れメールを確認しています（既読含む）...")
        sys.stdout.flush()
        _run_include_read(service, email_to_partner)
        print()
