...
| R6' | アシスト実行可（承認≠記帳） | 承認＝計画合意。Terminal でレール Go。gmail/sms OTP は Jarvis、アプリOTPはユーザー。無人無確認記帳なし |
...
- **自動振込の無人記帳は対象外**（送金アシスト Wave 0+: Preview→Go。詳細は `docs/KURASHIFT_送金アシスト_点検者向け仕様_20260814.md`）
...
- [ ] 承認しても無人記帳は走らない（Terminal Go＋照合が必要）
- [ ] money-ops に `rails[]`（OTP チャネル・status）が見える
