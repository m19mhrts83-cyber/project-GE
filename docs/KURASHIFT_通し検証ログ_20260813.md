...
| 方針 | **残してから寄せる**（月次引落＋バッファを各口座に残す）＋行き先も引落後バッファ確保 |
| SMBC刈谷 | いま 678,379円 → **目標 1,696,308円**（引落＋引落後10万） |
| 寄せ合計 | **約101.8万**（SBI本は Amex 用に50万残し・2.6万のみ／大垣・名銀・アパートは触らない） |
| money-ops | id `88bbac98-…` consulting「残してから寄せる」 |
| plan_ready_due | 2026-08-26 |
| 状態 | **承認待ち**。振込は手動 → done |
| やらない | 自動振込・元口座の月次引落を潰す寄せ・あかつき元本・SBIコア |
