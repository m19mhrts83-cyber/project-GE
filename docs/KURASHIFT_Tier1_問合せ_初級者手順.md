...
| **仕分け済（見た印）** | 「確認した」 | **status は info のまま**。候補に残る。次は図面・マイソク問合せ（まだ内見でも買い進めでもない）。Gmail 既読キュー |
...
1. 仕分け（確認した／対象外）→ 2. 詳細情報請求（問合せ）→ 3. 内見 → 4. 価格交渉 → 5. **買い進め**（買付証明・融資）
...
2. 上から1件ずつ  
   - 明らかに違う → **対象外**  
   - 良さそう → **問合せ**（今日の残り枠内）  
   - 迷う・Grok未調査 → **確認した** か、あとで戻る（候補に残る。内見にはならない）  
