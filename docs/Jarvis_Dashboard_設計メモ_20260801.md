...
| **最終同期** | ホーム上部に cloud／mac_morning を短く表示（詳細は折りたたみ） |
| **同期情報** | 折りたたみ。普段は邪魔にしない |

### 最新化の仕分け（2026-08-01）

- **夜間**: クラウドできるものだけ（GHA 維持。フル夜間クラウド化はしない）
- **朝（Mac 起床後・1日1回）**: `jarvis_triage_morning_open.py` が Vercel を開きつつ、裏で `jarvis_morning_mac_refresh.py`（catchup・Gmail 差分・dashboard push）
- **CHRLINE／Zaim**: 朝フルは任意（`--with-line`／週次・明示）
- 正本コマンド表: [`docs/運用コマンド一覧.md`](運用コマンド一覧.md)「最新化マトリクス」
