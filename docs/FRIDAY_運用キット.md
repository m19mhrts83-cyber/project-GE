...
### 設計ゲート（新自動化の着手前・2026-09-21）

完全自動から始めない。着手前に次を各1行書く（詳細: `jarvis-plan-accept.mdc`／Obsidian `仕事術・AI連携/20260921_AI導入_入口確認履歴停止の4点.md`）。

| 柱 | 例 |
|---|---|
| 入口データ | 何を読むか |
| 確認点 | 誰が了承するか（対外送信・オーナー確認は必須） |
| 変更履歴 | どこに残すか |
| 停止条件 | `disabled`／同意なし／競合・二重リスクで止める条件 |
