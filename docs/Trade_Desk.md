...
|---|---|---|
| **①-A** | 全体把握（週次） | 資産・ホーム合計 |
| **①-B** | 提案→相談→承認→実行準備 | テーマ・資金移動・相談 |
| **①-C** | 日常の短い回し方 | ホームに前面表示 |

**運用中の読み方**: アプリ `/guide`（構成ガイド）。サイドバーは **「運用」** グループ（①②③本線の外）。各メニューの役割も同画面。  
...
- **保険の契約者貸付残高**: 不動産頭金枠の把握用。ソニーは解約返戻と同ページから週次取得。`/portfolio` に表示。**貸付利率（年%）**は `config/liability_rates.yaml` ／ `SONYLIFE_POLICY_LOAN_RATE_PCT` 等（契約ごと・要確認）
- **不動産の正味利率**: 物件利回り − ローン金利。借入残高トラッカー連携後に③で一覧化予定
...
| **株式ウォッチ** | Theme 衛星の閾値監視（`/stock-watch`）。判断は Todoist **Theme株式** | 自動発注・NISAコア・Lab混線 |
...
関連シート（Numbers `260621` → KURASHIFT）:

| Numbers | アプリ |
|---|---|
| キャッシュフロー（表4収入 / 表5支出 / 表8評価・〜100歳） | `/lifeplan` **生涯CF**（閲覧。差分は前回版と比較）／`/lifeplan/analyze` **分析** |
| シングルインカム・表1（月別予算） | `/lifeplan/budget` **予算編成**（過去3〜4年＋当年月別） |
| αβγ・変えにくさ | `/lifeplan/abg` 支出の見方 |

年次更新は予算編成の `?mode=annual`。(a)実績反映 → (b)当年度予算 → (c)生涯CFへ反映 → (d)将来予測の修正と評価。セル直接編集は次フェーズ。書込は当面 Numbers＋Jarvis。
