...
| Numbers | アプリ |
|---|---|
| キャッシュフロー（表4収入 / 表5支出 / 表8評価・〜100歳） | `/lifeplan` **100歳計画**（閲覧。差分は前回版と比較） |
| シングルインカム・表1（月別予算） | `/lifeplan/budget` **予算編成**（過去3〜4年＋当年月別） |
| αβγ・変えにくさ | `/lifeplan/abg` 支出の見方 |

年次更新は予算編成の `?mode=annual`。(a)実績反映 → (b)当年度予算 → (c)100歳計画へ反映 → (d)将来予測の修正と評価。セル直接編集は次フェーズ。書込は当面 Numbers＋Jarvis。
