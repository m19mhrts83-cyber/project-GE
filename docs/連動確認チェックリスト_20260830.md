...
同時に進めるのは **Wave 1（T-05 のみ残）**。次の通しは **T-05 Tier1 実送信**。

1. このファイルと Notion `[T-05]` を開く
2. 下の「T-05 手順」と Tier1 初級者手順を上から触る（まず1件）
3. 使える → Jarvis に `T-05 意図OK`／違う → `T-05 変更要望: …`
...
合格メモ: vendors 最終同期 8/30・返信 deep link・判断ボタン（L-10）で担当待ちバッジまで確認。

### T-03 S1 → deals — **意図OK 2026-08-30**
...
6. **L-11**: 出所＝Grok調査結果／いまやること1行／**元メールを開く**・**掲載ページを開く**（無いときは無し表示）。`問合せ: none` なし

受け入れ条件（意図OKの目安）:
- 一宮・萩原町など `[Grok調査]` で、仲介メール必須に見えない
- スコア根拠1行が見える
- 一覧に出所チップ（Grok／メール）

メモ: L-12（神大家フォーム）同日OK。Grok依頼失敗のDB制約は修正済。判断履歴の失敗即時表示は **L-14**。

変更要望の索引 plan: `~/.cursor/plans/deals次アクション明示_f5a3b966.plan.md`
### T-04 deals 判断（送信なし）

1. **確認した（問合せへ）**＝仕分け（詳細問合せへ）。**内見にはならない**／**見送り**＝候補から外す
2. 問合せプレビュー（From=estate）まで。実送信は T-05
3. 内見は「内見にする」、買付・融資は「買付へ」（フェーズ5＝買い進め）
4. 候補タブ「整理候補を出す」で低スコア・エリア外・30日放置を一括見送り可（L-15）
5. 買い進め／進行中に **土地値%**（不明可）
...
1. 問合せ候補から1件選び、プレビューで From=estate・宛先・本文を確認できる
2. 確認後に送信キューへ入れ、`/jobs` で succeeded（または Mac worker 処理）を見られる
3. 案件の問合せステータスが進行中（awaiting_* 等）になる
4. （任意）同日あと最大で合計5件まで送れる
