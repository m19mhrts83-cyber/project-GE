#!/usr/bin/env python3
"""S9 管理会社 YAML → kurashift_re_mgmt_vendors 投影。

  cd ~/git-repos && set -a && source .env.jarvis_private && set +a
  ~/selenium_env/venv/bin/python scripts/jarvis_kurashift_mgmt_vendor_sync.py --dry-run
...
def vendor_row(v: dict[str, Any], *, synced_at: str) -> dict[str, Any]:
    ...
    row.update(alive_db_fields(v, kind="mgmt"))
    return row
...
def sync_vendors(*, apply: bool) -> dict[str, Any]:
    ...
    if not apply:
        print(f"# mgmt_vendor_sync dry-run: {len(rows)} rows")
    ...
    for i in range(0, len(rows), 50):
        batch = rows[i : i + 50]
        sb.table("kurashift_re_mgmt_vendors").upsert(batch, on_conflict="id").execute()
        upserted += len(batch)
    out["upserted"] = upserted
    out["synced_at"] = synced_at
    print(
        f"📎 mgmt_vendor_sync: upserted={upserted} "
