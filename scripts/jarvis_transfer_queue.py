...
LAUNCH_DIR = "215_kamiooya/C1_cursor/browser_automation"
...
def main() -> int:
    ...
    for r in rails:
        if not r.get("enabled", True):
            continue
        rid = r.get("id")
        launcher = r.get("launcher") or "(未配線)"
        print(f"- [{r.get('order')}] {r.get('label')}  {r.get('amount_jpy'):,}円")
        ...
        if launcher.endswith(".sh"):
            ...
            print(f"    cd ~/{LAUNCH_DIR.replace('215', 'git-repos/215')} && ./{base} --preview")
            print(f"    ./{base} --go{mid}")
        print("")
    if blocked:
        print("blocked:")
...
if __name__ == "__main__":
    # fix path display - use absolute helper
