...
このドキュメントでは、Gmail API を活用した取り組みと、トラブル対策を図解とステップでまとめています。
...
パートナーとのやり取り確認やいけともAIニュースで「Token has been expired or revoked」「invalid_grant」が発生した経験を踏まえ、**元の設定の不足点**と**今回の改善内容**をまとめます。Gmail API を円滑に利用するための周知資料として活用してください。

### 4.1 元々の設定で何が不足していたのか

| 不足していた点 | 説明 |
|----------------|------|
| **OAuth 同意画面が「テスト」のまま** | 「テスト」のままだと refresh token が**約 7 日**で失効する。対策を講じていても、この設定が「本番」になっていなければ、根本的にトークンが短命になる。 |
| **自動更新が実質動いていなかった** | launchd で 3 日ごとに実行する設定を入れていても、**Mac のスリープ**や**パス・作業ディレクトリのずれ**で、スクリプトが正しく動いていなかった可能性がある。 |
| **ログ確認の習慣がなかった** | 自動更新の成否は `~/Library/Logs/ai-news-token-refresh-error.log` で確認できるが、定期的に見ていなかったため、**失敗に気づかず**期限切れに至った。 |
| **パートナー用更新時の GMAIL_TOKEN_B64 更新漏れ** | パートナーやり取りで手動認証して token.json を更新したとき、**GitHub の GMAIL_TOKEN_B64 を更新し忘れる**と、いけともAIニュースの定期実行だけが古い token のまま失敗する。 |
| **「本番」にしたあとの再認証をしていなかった** | OAuth を「本番」に変更しただけでは、**既存の token.json は「テスト」時代の refresh token のまま**。本番用の refresh token を新規発行するには、一度 token を退避してから再認証する必要がある。 |

### 4.2 今回何を変更したことで、トークン切れを防げるようにしたのか

| 実施した対応 | 効果 |
|--------------|------|
| **OAuth 同意画面を「本番」に変更** | refresh token の有効期限が大幅に延びる（未使用で約 6 ヶ月、使用していれば実質ずっと）。 |
| **「本番」変更後に token を退避して再認証** | 古い「テスト」時代の refresh token を破棄し、**本番用の長寿命 refresh token** を新規発行した。 |
| **launchd の plist のパス・条件を見直し** | 作業ディレクトリや Python のパスが正しく参照されるように修正し、**3 日ごとの自動更新が確実に動く**ようにした。 |
| **ログ確認のルーチンを追加** | 定期的に `ai-news-token-refresh-error.log` を確認する習慣をつけ、**失敗を早期に検知**できるようにした。 |
| **パートナー更新時のチェックリスト整備** | パートナー用に token を更新したら、**必ず GMAIL_TOKEN_B64 も更新する**手順を明文化し、Cursor のルール（gmail-token-troubleshooting）で案内するようにした。 |

### 4.3 今後の運用で気をつけること

- **トークン更新時は「いけとも」「パートナー」「空室対策」の 3 系統すべてに影響**する。215 と DX 互助会の手順をまとめて確認する。
- **手動で token を更新したとき**は、GitHub の **GMAIL_TOKEN_B64** も忘れずに更新する（`GitHub Actions用_token更新手順.md` のステップ 2）。
- **数週間〜数ヶ月に一度**、`~/Library/Logs/ai-news-token-refresh-error.log` にエラーが出ていないか確認する。
...
- トークン更新時は「**いけともAIニュース**」「**パートナーやり取り**」「**空室対策メール送信**」のすべてに影響します。215 と DX 互助会の手順をまとめて確認することを推奨します。
