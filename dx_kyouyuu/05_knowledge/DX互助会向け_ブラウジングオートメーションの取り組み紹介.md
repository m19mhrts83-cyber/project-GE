...
| **tokairokin-transfer** | 東海労金インターネットバンキング | ログイン → 振込画面へ遷移 → フォーム自動入力 → OTP 入力で実行 |
...
一覧から **nichinoken-fetch** や **tokairokin-transfer** を選んで送信すると、コマンドの手順が開始されます。
...
**東海労金（tokairokin-transfer）の場合の例：**
...
### 4.3 東海労金の振込（tokairokin-transfer）
...
| **コマンド保存場所** | `.cursor/commands/`（リポジトリルート） | 同上 |
...
- 例：コマンドはリポジトリルートの `.cursor/commands/` に保存します（OneDrive に依存しない運用）。
...
| 種類 | 役割 | 例 |
|------|------|-----|
| **ルール** | ワークスペース全体で守る方針・場所の指定 | コマンドは `.cursor/commands/`（リポジトリルート）に保存する |
...
- コマンドファイル：`.cursor/commands/nichinoken-fetch.md`、`tokairokin-transfer.md`（リポジトリルート）
