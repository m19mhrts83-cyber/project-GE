...
1. **編集**  
   ...
   - ProgramCode: `git-repos/ProgramCode` を編集（旧 `~/ProgramCode` はワークスペースから外し、1つのリポジトリでコミットする）。
