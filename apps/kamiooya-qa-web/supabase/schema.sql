...
create unique index if not exists users_member_no_uidx
  on public.users (member_no) where member_no is not null;
...
-- 会員番号（登録時必須・既存ユーザーは NULL 可）
alter table public.users add column if not exists member_no text;
create unique index if not exists users_member_no_uidx
  on public.users (member_no) where member_no is not null;
