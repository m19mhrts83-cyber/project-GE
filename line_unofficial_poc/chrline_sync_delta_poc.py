...
E2EE 本文は端末に保存された自分用鍵（keyId）が揃っている必要がある（DESKTOP では未同期だと復号できないことがある）。
"""
...
def _msg_body_line_with_e2ee_register(
    cl, msg, op, registered: set[str], *, skip_register: bool
) -> str:
    gids = _collect_e2ee_group_mids(cl, op, msg)
    if gids:
