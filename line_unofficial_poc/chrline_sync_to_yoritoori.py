...
from chrline_md_utils import (
    ...
    format_line_heading as md_format_line_heading,
...
def _default_kamiooya_kanji_yoritoori_path() -> Path:
    ...
    if env:
        return Path(env).expanduser().resolve()
    if _ONEDRIVE_KAMIOOYA_KANJI_YORITOORI.is_file():
        return _ONEDRIVE_KAMIOOYA_KANJI_YORITOORI.resolve()
    return _REPO_KAMIOOYA_KANJI_YORITOORI.resolve()
...
_KAMIOOYA_KANJI_GROUP_TITLE_DEFAULT = "東海飲み会幹事やりとり"
_KAMIOOYA_KANJI_ORG_LABEL_DEFAULT = "東海飲み会幹事やりとり"
...
def _process_retry_queue(
    ...
    *,
    ...
    write_placeholder_on_exhaust: bool,
    verbose: bool,
) -> int:
    if not queue_items:
        return 0
    ...
    for dk, item in list(queue_items.items()):
        ...
        route, target = rt
        depth = fetch_depth if fetch_depth > 0 else retry_fetch_count
        msgs: list = []
        try:
            if depth > 300:
        ...
        if resolved_text is not None:
            ...
            if dry_run:
                print(
                    f"[dry-run][retry-resolved] ### {date_part}｜{route.org_label}｜{tag}｜…",
                    file=sys.stderr,
                )
            else:
                content = route.yoritoori_md.read_text(encoding="utf-8")
                content, replaced = upsert_resolved_block(
                    content,
                    date_part=date_part,
                    org_label=route.org_label,
                    tag=tag,
                ...
                if not replaced:
                    ...
                    content = md_insert_block_after_timeline_header(content, block)
                route.yoritoori_md.write_text(content, encoding="utf-8")
                seen_keys.add(dk)
                new_keys.add(dk)
            appended += 1
            done_keys.append(dk)
        ...
        item["next_retry_ts"] = now_ts + retry_interval_sec
        if attempts >= retry_max_attempts:
            if write_placeholder_on_exhaust:
                body_raw = str(
                    item.get("body_raw") or "[本文なし · E2EE 未復号またはコンパクトプレビューのみ]"
                )
                date_part = str(item.get("date_part") or "?")
                tag = str(item.get("tag") or target.recv_tag)
                if dry_run:
                    print(f"[dry-run][retry-fallback] {date_part} {tag}", file=sys.stderr)
                else:
                    content = route.yoritoori_md.read_text(encoding="utf-8")
                    content, replaced = upsert_resolved_block(
                        content,
                ...
                if verbose:
                    print(f"# retry queue exhausted (MD未確定): dk={dk[:24]}...", file=sys.stderr)
            if write_placeholder_on_exhaust or item.get("status") != "exhausted":
                done_keys.append(dk)
            if verbose and write_placeholder_on_exhaust and attempts >= retry_max_attempts:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="再照会キューの最大試行回数（超過時は exhausted としてキューに残す。--write-placeholder-on-exhaust で旧挙動）",
    )
    parser.add_argument(
        "--retry-fetch-count",
        type=int,
    ...
    parser.add_argument(
        "--write-placeholder-on-exhaust",
        action="store_true",
        help="再照会キュー枯渇時にプレースホルダーを MD へ確定追記する（旧挙動・緊急用）",
    )
    parser.add_argument(
        "--fetch-depth",
        type=int,
        default=0,
        help="再照会・修復時の履歴取得深度（0 なら --retry-fetch-count のみ）",
    )
    return parser
...
def run(args: argparse.Namespace, *, client=None) -> int:
    ...
    if preset == "line-default":
        if args.kamiooya_kanji_yoritoori_md is None:
            kpath = _default_kamiooya_kanji_yoritoori_path()
        else:
            kpath = args.kamiooya_kanji_yoritoori_md.expanduser().resolve()
        routes.append(
            _YoritooriRoute(kpath, args.kamiooya_kanji_org_label, [])
        )
    ...
    if preset == "line-default":
        ...
        for route in routes:
            for t in route.targets:
                known_mids.add(t.needle)
        for dct in load_direct_chat_targets(exclude_mids=known_mids):
            tgt = _YoritooriTarget(
                needle=dct.chat_mid,
                recv_tag=f"{dct.peer_label}から返信（LINE 1:1・CHRLINE sync）",
                send_tag="自分から送信（LINE 1:1・CHRLINE sync）",
            )
            merged = False
            for route in routes:
                if route.yoritoori_md == dct.yoritoori_md:
                    route.targets.append(tgt)
                    merged = True
                    break
            if not merged:
                routes.append(_YoritooriRoute(dct.yoritoori_md, dct.org_label, [tgt]))
    ...
    try:
        ...
        if preset in ("line-default", "tcell-both"):
            ...
            if preset == "line-default":
                yuki_needle = needle or _YUKI_CHAT_NEEDLE_DEFAULT
                if yuki_needle not in {t.needle for t in tcell_route.targets}:
                    tcell_route.targets.append(
                        _YoritooriTarget(
                            needle=yuki_needle,
                            recv_tag=f"{args.peer_label}から返信（LINE 1:1・CHRLINE sync）",
        ...
        if preset == "line-default":
            kamiooya_route = routes[2]
            kkneedle = (args.kamiooya_kanji_group_chat_mid or "").strip() or (
                os.environ.get("LINE_KAMIOOYA_KANJI_GROUP_CHAT_MID") or ""
            ).strip()
            if not kkneedle:
                kkneedle = _resolve_group_mid_by_title(
                    cl, (args.kamiooya_kanji_group_title or "").strip()
                ) or ""
            if not kkneedle:
                print(
                ...
                kpeer = (
                    (args.kamiooya_kanji_group_peer_label or "").strip()
                    or (args.kamiooya_kanji_group_title or "").strip()
                    or _KAMIOOYA_KANJI_GROUP_TITLE_DEFAULT
                )
                kamiooya_route.targets.append(
                    _YoritooriTarget(
                        needle=kkneedle,
                        recv_tag=f"{kpeer}（LINEグループ・CHRLINE sync・受信）",
                        send_tag="自分から送信（LINEグループ・CHRLINE sync）",
                    )
                )
        ...
        if args.retry_queue and retry_queue_items:
            appended += _process_retry_queue(
                ...
                write_placeholder_on_exhaust=bool(args.write_placeholder_on_exhaust),
        ...
        if pending_sync_placeholders:
            ...
            for p in pending_sync_placeholders:
                ...
                if args.retry_queue and p.dk:
                    ...
                    wrote_pending += 1
                elif args.write_placeholder_on_exhaust:
                    heading = md_format_line_heading(
                        p.date_part,
                        p.route.org_label,
                        p.tag,
                        body_for_tail=p.body_raw,
                    )
                    block = f"""
...
"""
            ...
            if wrote_pending and args.verbose:
                print(f"# sync プレースホルダー保留分を処理: {wrote_pending} 件", file=sys.stderr)
